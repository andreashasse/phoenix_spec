...
3. For each route, extract the 3-arity function spec from the controller's type info
...
- Controller actions use **3-arity** `(path_args, headers, body)` and return `{status_code, headers, body}`
